...
test.group('Custom Error Reporter - 404 for params validation', () => {
  ...
  test('PUT /api/admin/youtube-videos/:id should return 404 for non-existent id', async ({ client }) => {
  ...
  test('DELETE /api/admin/youtube-videos/:id should return 404 for non-existent id', async ({ client }) => {
...
test.group('Custom Error Reporter - 422 for body validation', () => {
  test('POST /api/admin/games should return 422 for non-existent genreIds in body', async ({ client, assert }) => {
    ...
    const genreError = body.errors.find(err => err.field.includes('genreIds'))
    assert.exists(genreError)
    assert.equal(genreError.rule, 'database.exists')
  })

  test('POST /api/admin/games should return 422 for non-existent platformIds in body', async ({ client, assert }) => {
    const response = await client.post('/api/admin/games').json({
      name: 'Valid Game Name',
      description: 'Valid description with enough characters',
      releaseDate: '2024-01-01',
    ...
    const platformError = body.errors.find(err => err.field === 'platformIds.0')
    assert.exists(platformError)
    assert.equal(platformError.rule, 'database.exists')
  })

  test('POST /api/admin/games should return 422 for invalid name length', async ({ client, assert }) => {
    const response = await client.post('/api/admin/games').json({
      name: 'X',
      description: 'Valid description with enough characters',
      releaseDate: '2024-01-01',
    ...
    const nameError = body.errors.find(err => err.field === 'name')
    assert.exists(nameError)
    assert.equal(nameError.rule, 'minLength')
    assert.include(nameError.message, 'at least')
  })

  test('POST /api/admin/youtube-videos should return 422 for non-existent gameId in body', async ({ client, assert }) => {
    ...
    const gameError = body.errors.find(err => err.field === 'gameId')
    assert.exists(gameError)
    assert.equal(gameError.rule, 'database.exists')
  })

  test('PUT /api/admin/games/:id should return 422 for non-existent genreIds in body (but existing game)', async ({ client, assert }) => {
...
test.group('Custom Error Reporter - Mixed scenarios', () => {
  test('should return 422 when both params and body have validation errors', async ({ client, assert }) => {
    const response = await client.put('/api/admin/games/999999').json({
      name: 'X',
      genreIds: [999999]
    })
    ...
    const hasParamsError = body.errors.some(err => err.field.includes('params.'))
    const hasBodyError = body.errors.some(err => !err.field.includes('params.'))
