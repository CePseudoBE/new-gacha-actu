...
export default class ArticlesController {
  ...
  async index(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, articles)
  }
  ...
  async show(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, article)
  }
  ...
  async store(ctx: HttpContext) {
    ...
    return ResponseService.created(ctx, article, 'Article créé avec succès')
  }
  ...
  async update(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, article, 'Article mis à jour avec succès')
  }

  async destroy(ctx: HttpContext) {
    const { params: validatedParams } = await ctx.request.validateUsing(articleParamsValidator)
    await this.articleService.deleteArticle(validatedParams.id)
    return ResponseService.success(ctx, 'Article supprimé avec succès')
  }
}
