...
export default class PlatformsController {
  ...
  async index(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, platforms)
  }
  ...
  async show(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, platform)
  }
  ...
  async store(ctx: HttpContext) {
    ...
    return ResponseService.created(ctx, platform, 'Platform créée avec succès')
  }
  ...
  async update(ctx: HttpContext) {
    ...
    return ResponseService.ok(ctx, platform, 'Platform mise à jour avec succès')
  }

  async destroy(ctx: HttpContext) {
    const { params: validatedParams } = await ctx.request.validateUsing(platformParamsValidator)
    await this.platformService.deletePlatform(validatedParams.id)
    return ResponseService.success(ctx, 'Platform supprimée avec succès')
  }
}
